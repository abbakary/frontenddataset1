...
export default function RoleBasedNav({ currentPath }) {
  ...
  const userRole =
    normalizedRole === "superadmin" ? "super_admin" : normalizedRole || "viewer";
  const navItemsRaw = ROLE_NAV_ITEMS[userRole] || ROLE_NAV_ITEMS.viewer;

  // Remove duplicates based on path-section-label combination
  const navItems = navItemsRaw.filter((item, index, arr) => {
    const key = `${item.path}-${item.section}-${item.label}`;
    return (
      arr.findIndex(
        (candidate) =>
          `${candidate.path}-${candidate.section}-${candidate.label}` === key
      ) === index
    );
  });
  ...
  const handleNavigate = (path, section) => {
    if (section) {
      ...
      window.dispatchEvent(
        new CustomEvent("dashboard:section", { detail: section })
      );
    }
    navigate(path);
    handleRoleClose();
  ...
  return (
    ...
    >
      ...
      <Box sx={{ mb: 3, pb: 2, borderBottom: "1px solid #eee" }}>
        ...
        {/* Role Switcher Menu */}
        <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleRoleClose}>
